...
## Advanced Imitation Learning - Integration and Optimization
...
3. In the container, build the ROS workspace:
   ```bash
   cd /workspace && colcon build --packages-select lab_11_3
...
To run the ROS node:
```bash
ros2 run lab_11_3 lab_11_3_node
```

To launch with the launch file:
```bash
ros2 launch lab_11_3 lab_11_3.launch.py
```

### Expected Output

The node should publish messages to the `lab_11_3/result` topic with processing results.
...
**Lab created**: 2026-01-05T23:06:04.273294

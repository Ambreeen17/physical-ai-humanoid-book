...
## Introduction to Sensors & Actuators - Basic Concepts
...
3. In the container, build the ROS workspace:
   ```bash
   cd /workspace && colcon build --packages-select lab_3_1
...
To run the ROS node:
```bash
ros2 run lab_3_1 lab_3_1_node
```

To launch with the launch file:
```bash
ros2 launch lab_3_1 lab_3_1.launch.py
```

### Expected Output

The node should publish messages to the `lab_3_1/result` topic with processing results.
...
**Lab created**: 2026-01-05T21:42:50.789777

...
## Intermediate Sensors & Actuators - Implementation
...
3. In the container, build the ROS workspace:
   ```bash
   cd /workspace && colcon build --packages-select lab_3_2
...
To run the ROS node:
```bash
ros2 run lab_3_2 lab_3_2_node
```

To launch with the launch file:
```bash
ros2 launch lab_3_2 lab_3_2.launch.py
```

### Expected Output

The node should publish messages to the `lab_3_2/result` topic with processing results.
...
**Lab created**: 2026-01-05T21:42:50.801661

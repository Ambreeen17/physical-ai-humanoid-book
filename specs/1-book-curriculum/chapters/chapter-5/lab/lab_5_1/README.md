...
## Introduction to Computer Vision for Robotics - Basic Concepts
...
3. In the container, build the ROS workspace:
   ```bash
   cd /workspace && colcon build --packages-select lab_5_1
...
To run the ROS node:
```bash
ros2 run lab_5_1 lab_5_1_node
```

To launch with the launch file:
```bash
ros2 launch lab_5_1 lab_5_1.launch.py
```

### Expected Output

The node should publish messages to the `lab_5_1/result` topic with processing results.
...
**Lab created**: 2026-01-05T23:06:03.557885

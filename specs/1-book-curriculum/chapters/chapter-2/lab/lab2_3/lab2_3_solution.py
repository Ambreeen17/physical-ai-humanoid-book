#!/usr/bin/env python3
"""
Lab 2.3: Dynamics Simulation in MuJoCo - SOLUTION
...
Solution:
- Mass matrix using Lagrangian derivation
- Coriolis matrix using Christoffel symbols
- Passive dynamics simulation comparison
"""
...
def compute_mass_matrix(q: List[float], m1: float, m2: float,
    ...
    The mass matrix encodes the kinetic energy of the system:
        T = 0.5 * qd^T * M(q) * qd

    M(q) = | M11  M12 |
           | M21  M22 |

    Elements derived from Lagrangian mechanics:
    - M11: Total inertia affecting joint 1 (includes coupling from joint 2)
    - M12, M21: Coupling terms (asymmetric motion of link 2 affects joint 1)
    - M22: Inertia of link 2 about its own joint
...
def compute_coriolis_matrix(q: List[float], qd: List[float],
    ...
    - Centrifugal: proportional to qd_i^2 (forces from rotation)
    - Coriolis: proportional to qd_i * qd_j (coupling between joints)

    These terms arise because the kinetic energy depends on configuration,
    so derivatives with respect to q create velocity-dependent forces.

    Using Christoffel symbols derived from the mass matrix:
    C_ij = sum_k Gamma_ijk * qd_k
    where Gamma_ijk = (dM_ij/dq_k + dM_ik/dq_j - dM_jk/dq_i) / 2

    Simplified expressions for 2-link arm:
    ...
    # These arise from the configuration-dependence of the kinetic energy
...
def compute_analytical_acceleration(q: List[float], qd: List[float],
    ...
    For zero torque, the dynamics equation simplifies to:
        M(q)*qdd + C(q, qd)*qd = 0

    Solving for acceleration:
        qdd = -M(q)^(-1) * C(q, qd) * qd

    This represents the passive dynamics of the arm - how it moves
    under gravity and coupling forces with no motor input.
    ...
    # Compute qdd = -M^(-1) * C * qd (zero torque case)
...
def run_mujoco_simulation(xml_path: str, initial_q: List[float],
    ...
    MuJoCo uses implicit integration for stability with stiff constraints.
    The simulation advances using mj_step which internally uses a
    semi-implicit Euler method with iterative constraint satisfaction.
...
def compute_analytical_trajectory(initial_q: List[float], initial_qd: List[float],
    ...
    Note: Euler integration is approximate; higher-order methods (RK4)
    would give better accuracy for long simulations.
...
def main():
    ...
    """
    print("=" * 60)
    print("Lab 2.3: Dynamics Simulation in MuJoCo - SOLUTION")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Lab 2.3 Complete! Solution verified.")
    print("=" * 60)
